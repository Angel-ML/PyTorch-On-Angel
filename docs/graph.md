...
- Angel PS resource: in order to ensure that Angel does not hang up, it is necessary to configure memory that is about twice the size of the model. The formula for calculating the size of the graph feature is: node_num * feature_dim * 4Byte, such as: 1kw nodes, feature_dim is 100, the size of graph feature is about 4G, then set ps.instances=3, ps.memory=4G is reasonable(total memory is 2~3 times of data saved in ps.). Of course, this is only a simple case. In many algorithms, there are many types of data saved in ps, such as: edge, node feature, node label etc.
	- `GraphSage`: edge, node feature, node label, the model size is: (edge_num * 2 * 8Byte + node_num * feature_dim * 4Byte + the num of node with label * 4Byte)
	- `RGCN`: edge with type, node feature, node label, the model size is: (edge_num * 2 * 8Byte + edge_num * 4Byte + node_num * feature_dim * 4Byte + the num of node with label * 4Byte)
	- `DGI`: edge, node feature, the model size is: (edge_num * 2 * 8Byte + node_num * feature_dim * 4Byte)
...
1. **Generate pytorch sciprt model**
    First, go to directory of python/graph and execute the following command:
    ```$xslt
    python edgeprop_onehop.py --input_dim 23 --edge_input_dim 7 --hidden_dim 128 --output_dim 7 --output_file edgeprop_eth_onehop_1.5.pt
    ```
    This script utilizes [TorchScript](https://pytorch.org/docs/stable/jit.html) to generate a model file which contains the dataflow graph of edgeProp. After that, you will obtain a model file named "edgeProp.pt". Where edge_input_dim is the dimention of edge feature, other parameters are same as GraphSAGE.
...
2. **Submit model to cluster**
   ...
   $SPARK_HOME/bin/spark-submit \
          ...
          --executor-cores 1 \
          --executor-memory 5g \
          --class com.tencent.angel.pytorch.example.supervised.EdgePropGCNExample \
          ./pytorch-on-angel-${VERSION}.jar \   # jar from Compiling java submodule
          edgePath:$edgePath featurePath:$featurePath labelPath:$labelPath\
          torchModelPath:rgcn_mutag.pt featureDim:32 stepSize:0.01\
          optimizer:adam numEpoch:10 testRatio:0.5\
