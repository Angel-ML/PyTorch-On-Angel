...
import com.tencent.angel.pytorch.graph.gcn.GAT
...
object GATExample {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = EntryUtils.start(mode, torchModelPath, "Graph Attention Network", actionType)
    ...
    gat.setPSPartitionNum(psPartitionNum)
    gat.setPartitionNum(dataPartitionNum)
    ...
    gat.setSaveCheckpoint(saveCheckpoint)
    ...
    val (model, graph) = gat.initialize(edges, features, labels, testLabels)
    ...
    if (predictOutputPath.nonEmpty) {
      val predict = gat.genLabels(model, graph)
      GraphIO.save(predict, predictOutputPath, seq = Delimiter.SPACE_VAL)
      println(s"save predict cost: ${(System.currentTimeMillis() - start)/1000}s")
    }

    start = System.currentTimeMillis()
    if (embeddingPath.nonEmpty) {
      val embedding = gat.genEmbedding(model, graph)
    ...
    if (Constants.TRAIN.equals(actionType) && outputModelPath.nonEmpty) {
      ...
      if (config.model.get(Constants.INPUT_FIELD_NUM).toString.toInt  > 0) {
