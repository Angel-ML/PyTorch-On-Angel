...
class GAT extends GCN {
  ...
  def makeGraph(edges: DataFrame, model: GNNPSModel, labelDF: Option[DataFrame],
    ...
    val gcnGraph =
      adjGraph.map(_.toSemiGATPartition(model, $(torchModelPath), $(useSecondOrder), $(testRatio), $(numLabels)))

    // build GCN graph partitions
    //    val gcnGraph = adjGraph.map(_.toSemiGATPartition(model, $(torchModelPath), $(testRatio)))
    gcnGraph.persist($(storageLevel))
    gcnGraph.count()
    adjGraph.unpersist(true)

    implicit val encoder = org.apache.spark.sql.Encoders.kryo[GATPartition]
  ...
  def fit(model: GNNPSModel, graph: Dataset[_], checkPointPath: String = null): Unit = {
    ...
    val (trainSize, testSize) = graph.rdd.map(_.asInstanceOf[GATPartition].getTrainTestSize)
    ...
    for (curEpoch <- 1 to $(numEpoch)) {
      ...
      val res = graph.rdd.map(_.asInstanceOf[GATPartition]
