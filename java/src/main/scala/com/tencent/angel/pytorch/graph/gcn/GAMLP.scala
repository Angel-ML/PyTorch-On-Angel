...
class GAMLP extends GNN with HasTestRatio with HasValidate with HasUseSharedSamples {
  ...
  def makeGraph(edges: DataFrame, model: GNNPSModel, labelDF: Option[DataFrame],
    ...
    println("GAMLP has no need to init neibor matrix to ps.")
  ...
  override def showSummary(model: GNNPSModel, graph: Dataset[_]): Unit = {
