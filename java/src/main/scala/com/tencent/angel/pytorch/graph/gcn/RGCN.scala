...
import com.tencent.angel.pytorch.params.HasTestRatio
import org.apache.spark.sql.types.{LongType, StringType, StructField, StructType}
import org.apache.spark.sql.{DataFrame, Dataset, Row, SparkSession}

class RGCN extends GCN with HasTestRatio {
  ...
  def makeGraph(edges: DataFrame, model: GNNPSModel): Dataset[_] = {
    val adjGraph = edges.select("src", "dst", "type").rdd
      ...
      .mapPartitionsWithIndex((index, it) => Iterator(GraphAdjTypePartition.apply(index, it)))
    ...
    val rgcnGraph = adjGraph.map(_.toSemiRGCNPartition(model, $(torchModelPath), $(testRatio)))
  ...
  def genEmbedding(model: GNNPSModel, graph: Dataset[_]): DataFrame = {
    val ret = graph.rdd.flatMap(_.asInstanceOf[GNNPartition]
      .genEmbedding($(batchSize) * 10, model, $(featureDim), $(numSamples), graph.rdd.getNumPartitions, false))
  ...
  def genLabels(model: GNNPSModel, graph: Dataset[_]): DataFrame = {
    val ret = graph.rdd.flatMap(_.asInstanceOf[GNNPartition]
      .genLabels($(batchSize) * 10, model, $(featureDim), $(numSamples), graph.rdd.getNumPartitions, false))
