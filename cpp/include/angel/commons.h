...
#define TORCH_OPTION_INT64 TORCH_OPTION(torch::kInt64, false)

#define TORCH_OPTION_INT32 TORCH_OPTION(torch::kInt32, false)

#define TORCH_OPTION_FLOAT TORCH_OPTION(torch::kFloat, false)

#define TORCH_OPTION_FLOAT_GRAD TORCH_OPTION(torch::kFloat, true)
...
#define __DEFINE_ARRAY_TENSOR_DIM1(array, dim, type, is_grad)                  \
  auto array##_tensor =                                                        \
      torch::from_blob(array, {dim}, TORCH_OPTION(type, is_grad));

#define __DEFINE_ARRAY_TENSOR_DIM2(array, dim1, dim2, type, is_grad)           \
  auto array##_tensor =                                                        \
      torch::from_blob(array, {dim1, dim2}, TORCH_OPTION(type, is_grad));
...
#define __DEFINE_JARRAY_TENSOR_DIM1(_jarray, dim, type, is_grad)               \
  auto _jarray##_tensor =                                                      \
      torch::from_blob(_jarray##_cptr, {dim}, TORCH_OPTION(type, is_grad));

#define __DEFINE_JARRAY_TENSOR_DIM2(_jarray, dim1, dim2, type, is_grad)        \
  auto _jarray##_tensor = torch::from_blob(_jarray##_cptr, {dim1, dim2},       \
                                           TORCH_OPTION(type, is_grad));

#define DEFINE_JARRAY_TENSOR_DIM_INT64(_jarray)                                \
  __DEFINE_JARRAY_TENSOR_DIM1(_jarray, env->GetArrayLength(_jarray),           \
                              torch::kInt64, false)
...
//  auto _jarray##_option =
//  torch::TensorOptions().dtype(type).requires_grad(true);\ auto
//  _jarray##_tensor = torch::from_blob(_jarray##_cptr,
//  {env->GetArrayLength(_jarray)}, _jarray##_option);
//
//#define DEFINE_TORCH_TENSOR_SCALA(_jprimitive, type) \
//  auto _jprimitive##_option =
//  torch::TensorOptions().dtype(type).requires_grad(false);\ auto
//  _jprimitive##_tensor = torch::from_blob(&_jprimitive, {1},
//  _jprimitive##_option);

#define DEFINE_JFLOATARRAY(_carray_ptr, len)                                   \
  jfloatArray _carray_ptr##_jarray = env->NewFloatArray(len);                  \
  env->SetFloatArrayRegion(_carray_ptr##_jarray, 0, len,                       \
                           reinterpret_cast<float *>(_carray_ptr));

#define DEFINE_JINTARRAY(_carray_ptr, len)                                     \
  jintArray _carray_ptr##_jarray = env->NewIntArray(len);                      \
  env->SetIntArrayRegion(_carray_ptr##_jarray, 0, len,                         \
                         reinterpret_cast<int *>(_carray_ptr));

#define DEFINE_JLONGARRAY(_carray_ptr, len)                                    \
  jlongArray _carray_ptr##_jarray = env->NewLongArray(len);                    \
  env->SetLongArrayRegion(_carray_ptr##_jarray, 0, len,                        \
                          reinterpret_cast<int64_t *>(_carray_ptr));
...
#define DEFINE_PRIMITIVE_ARRAYS6(jarray1, jarray2, jarray3, jarray4, jarray5,  \
                                 jarray6)                                      \
...
#define RELEASE_PRIMITIVE_ARRAYS6(jarray1, jarray2, jarray3, jarray4, jarray5, \
                                  jarray6)                                     \
...
#define DEFINE_PRIMITIVE_OBJECT_ARRAY(jobjectarray)                            \
  ...
  for (int i = 0; i < env->GetArrayLength(jobjectarray); i++)                  \
    jobjectarray##_cptrs[i] = env->GetPrimitiveArrayCritical(                  \
        (jarray)env->GetObjectArrayElement(jobjectarray, i), &is_copy);

#define RELEASE_PRIMITIVE_OBJECT_ARRAY(jobjectarray)                           \
  for (int i = 0; i < env->GetArrayLength(jobjectarray); i++)                  \
    env->ReleasePrimitiveArrayCritical(                                        \
        (jarray)env->GetObjectArrayElement(jobjectarray, i),                   \
        jobjectarray##_cptrs[i], 0);

// copy data from tensor.grad to jarray
#define __SET_FLOAT_GRAD(jarray, start, length)                                \
  env->SetFloatArrayRegion(jarray, start, length,                              \
                           jarray##_tensor.grad().data<float>());
